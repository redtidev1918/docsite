...
- [文件与命名规范](/CONVENTIONS.md)：中英目录、下载页生成链路、一次性/阶段文档生命周期
...
部署 workflow、长期停在旧版本的下载页。docsite 只把壳子收进模板，把 Markdown 留给
仓库自己。
...
| 不下发内容 | 下载页是唯一的例外：它由 release 数据生成，手写必然过期 |
| 不做批量升级 | 每个仓库各跑一次 `update`，diff 可见可控 |
